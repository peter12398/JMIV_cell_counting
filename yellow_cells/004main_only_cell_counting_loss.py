...
from skimage.morphology import dilation, erosion, disk, square, opening, closing, label
import glob
...
parser.add_argument("--exp_name", default="newCCCLayer_RMSProp_new_def_grad_minus_upstream_interactive_N_2_cache_Noclip_yellow_cells_reduceLrPlateau_with_zeros_official_testsplit".format(PATCH_SIZE), type=str, help="experiment name")
parser.add_argument("--model_weight_path", default="/cluster/CMM/home/xfliu/workspace/JMIV_counting_cells/best_model_newCCCLayer_RMSProp_new_def_grad_minus_upstream_interactive_N_2_cache_Noclip_yellow_cells_reduceLrPlateau_with_zeros_official_testsplit.h5".format(PATCH_SIZE), type=str, help="experiment name")
...
def custom_h_rec_and_exact_cc(xinput):
    ...
    LAMDA = 100.
    CC_ = tf.math.minimum(CC, LAMDA*(tf.reduce_max(dxh_dh) - tf.reduce_min(dxh_dh))*CC)
    print('CC',CC_)
    if args.mode == "test":
        return [CC_,Detection], grad
    else:
        return CC_, grad
    """
...
class Dataset:
    ...
    def __getitem__(self, i):
        ...
        if not image_name in self.imlabels_cache_dict.keys():
            ...
            imLabelsLarge = io.imread(self.images_fps[i].replace("images", "labels"))#io.imread(labels_dir+imname+'.png')
        ...
        mask = imLabels #io.imread(self.masks_fps[i])
...
def detectCells_from_predicted_detection(Detection, CC):
    
    if CC:
        imDetec = dilation(Detection[0,:,:,0], square(3))*255
        ncells = CC
    else:
        imDetec = np.zeros(Detection.shape)
        ncells = 0

    return ncells, imDetec
...
class H_maxima_model:
    def __init__(self, input_shape, train_dataloader, valid_dataloader, test_dataloader, test_dataset, EPOCHS=100, BATCH_SIZE = 16, loss = 'binary_crossentropy', loss_weights = None , lr = 0.01,  IMAGE_SAVE_PATH = "/home/xiaohu/PythonProject/DIMA_comtage_cellule/visualize_main", MODE = "post_processor", metrics = None, resume = False) -> None:
        ...
        self.nn, self.nn_h, self.nn_detection = self.get_simple_model(self.input_shape)
        self.nn.summary()
        ...
        #self.nn.compile(loss=self.loss,loss_weights = self.loss_weights, optimizer=tf.keras.optimizers.Adam(learning_rate=self.lr, clipvalue=0.5), metrics = self.metrics)
    ...
    def get_simple_model(self, input_shape):
        ...
        if args.mode == "test":
            [NCC,Detection]=tf.keras.layers.Lambda(custom_h_rec_and_exact_cc)([xinput,xh])
            detected_diff = NCC #modeltoCount(np.expand_dims(np.expand_dims(imMax,axis=0),axis=-1)).numpy()[0]
            return tf.keras.Model(xin,detected_diff), tf.keras.Model(xin,xh),tf.keras.Model(xin,Detection)
        else:
            NCC=tf.keras.layers.Lambda(custom_h_rec_and_exact_cc)([xinput,xh])
            detected_diff = NCC
            return tf.keras.Model(xin,detected_diff), tf.keras.Model(xin,xh),tf.keras.Model(xin,detected_diff)
    ...
    def predict_detection(self, X):
        return self.nn_detection.predict(X)
    ...
    def test(self,):      
        ...
        average_relative_error_list_ARE = []
        ...
        for i in ids:
            ...
            #count_err_diff_list.append(ntruth - ndetec)
            count_err_nomerator_list.append(np.abs(ntruth))
            if np.abs(ntruth)!= 0:
                average_relative_error_list.append((ntruth - ndetec)/np.abs(ntruth))
                average_relative_error_list_ARE.append(np.abs(ntruth - ndetec)/np.abs(ntruth))

            Detection = self.predict_detection(image_expanded)
            ncells, imDetec = detectCells_from_predicted_detection(Detection, ndetec)

            assert ncells == ndetec
            ...
            imLabels = mask
            #imLabels = erosion(imLabels, square(3))

            binary_image = np.array((imLabels*255 > 125), dtype=np.uint8)
            label_im = label(binary_image)
            props = regionprops(label_im)
            ...
            idxs = np.where(imLabels_resized > 0)
            for k in range(3):
                #if not NORMALISE01:
                #    imTruth[:,:,k] = image/255 #im #im/255
                #else:
                imTruth[:,:,k] = 0.299*image[:,:,0]+0.587*image[:,:,1]+0.114*image[:,:,2] #image[:,:,k] #im/255
                if k == 0:
            ...
            for k in range(3):
                ...
                #else:
                imDetecCol[:,:,k] = 0.299*image[:,:,0]+0.587*image[:,:,1]+0.114*image[:,:,2] #image[:,:,k] #im/255
                if k == 0:
            ...
            plt.imsave(save_path+"/{}_imDetecCol_{}_prh_{}.png".format(imname,ndetec, str(pr_h)), imDetecCol)
            plt.imsave(save_path+"/{}_label_im.png".format(imname), label_im)
            plt.imsave(save_path+"/{}_Detection_pred.png".format(imname), Detection[0,:,:,0])
        ...
        print("MPE(%) =:", np.average(average_relative_error_list))
        print("MAE =:", np.average(count_err_diff_list))
        print("ARE =:", np.average(average_relative_error_list_ARE))
        print("TRE =:", np.sum(count_err_diff_list)/ np.sum(count_err_nomerator_list))
